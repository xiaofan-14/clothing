...
const bottomTabs = ref([
  { id: 1, icon: House, active: true },
  { id: 2, icon: ShoppingCart, badge: 2, active: false },
  { id: 3, icon: Heart, active: false },
  { id: 4, icon: UserRound, active: false },
])

function setActiveTab(tabId: number) {
  router.push('/')
  bottomTabs.value.forEach((tab) => (tab.active = tab.id === tabId))
}
</script>
...
<template>
  ...
  >
    <button
      ...
      :class="tab.active ? 'bg-white/10 text-white' : 'text-gray-400'"
